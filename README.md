...
The core concept of _TreeTime_ is the idea to use linked trees. Linked trees are separate trees that share the same data. One piece of data (a _node_) can be in several trees at the same time, but in different place of the tree.    
...
If you, for instance, organise information about animals, you might want to see taxonomy (kingdom/class/family/species) in one tab, habitat (continent/country/area) in another, and switch between them. If you organise tasks, you could switch between a year/quarter/week/day breakdown, a company/department/team/person tree, and a product/project/package/task overview.
...
- Windows, Linux: Go to [github.com/jkanev/treetime/releases/tag/v0.0](https://github.com/jkanev/treetime/releases/tag/v0.0) and download a zipped package for Windows 10, 64 bit, or for Linux 64 bit from there. Unzip it into your program directory and run _TreeTime_ or _TreeTime.exe_ from the new folder.
...
2. If you don't have it yet, install PyQt5: On the command line, type "pip install PyQt5"
3. Type "pip install TreeTime" (or "pip3 install TreeTime" depending on your installation).
...
- Done October 2018: Re-furbished the GUI and removed a couple of bugs. Slighty changed the data file format. Implemented theme selection. Tested pyqtdeploy for deployment instead of pyinstaller.
